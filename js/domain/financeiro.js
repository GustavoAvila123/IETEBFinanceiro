...
function filtrarEntradasPorPeriodo(entradas, deISO, ateISO) {
  return entradas.filter(
    (i) => i.dataDeposito && i.dataDeposito >= deISO && i.dataDeposito <= ateISO
  );
}

function filtrarSaidasPorPeriodo(saidas, deISO, ateISO) {
  return saidas.filter((i) => i.data && i.data >= deISO && i.data <= ateISO);
}
...
function somarPorFormaPagamento(itens, tipo) {
  return itens.filter((i) => i.formaPagamento === tipo).reduce((s, i) => s + parseBRL(i.valor), 0);
}

function somarPorBancarios(itens) {
  return (
    somarPorFormaPagamento(itens, 'Pix') +
...
function calcularSaldoSeparado(
  entradas,
  saidas,
  saldoAberturaDinheiro = 0,
  saldoAberturaConta = 0
) {
